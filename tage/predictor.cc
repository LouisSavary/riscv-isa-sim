#ifndef _PREDICTOR_CC_
#define _PREDICTOR_CC_
...
int GI_bis[NHIST + 1];			 // indexes to the different tables are computed only once, used in prepred
uint GTAG_bis[NHIST + 1];		 // tags for the different tables are computed only once, used in prepred
// bool update_since_last_pred; // used to differenciate prediction from pre_prediction
...
int predictorsize()
{
	...
	int PREDICTOR::bindex(UINT64 PC)
	{
	...
	int PREDICTOR::F(long long A, int size, int bank)
	{
		int A1, A2;
		A = A & ((1 << size) - 1);
		A1 = (A & ((1 << logg[bank]) - 1));
	...
	int PREDICTOR::gindex(unsigned int PC, int bank, long long hist,
						 folded_history *ch_i)
	{
		int index;
	...
	//  tag computation
	uint16_t PREDICTOR::gtag(unsigned int PC, int bank, folded_history *ch0,
								folded_history *ch1)
	{
	...
	void PREDICTOR::ctrupdate(int8_t &ctr, bool taken, int nbits)
	{
		if (taken)
		{
			if (ctr < ((1 << (nbits - 1)) - 1))
	...
	void PREDICTOR::Tagepred(UINT64 PC, bool isprepred=false)
	{
		int *GI_loc = GI_bis;
		uint *GTAG_loc = GTAG_bis;
		if (isprepred)
		{
			GI_loc = GI;
			GTAG_loc = GTAG;
		...
		{
			GI_loc[i] = gindex(PC, i, phist, ch_i);
			GTAG_loc[i] = gtag(PC, i, ch_t[0], ch_t[1]);
			GTAG_loc[i + 1] = GTAG_loc[i];
			GI_loc[i + 1] = GI_loc[i] ^ (GTAG_loc[i] & ((1 << LOGG) - 1));
		}
		...
		{
			if (NOSKIP[i])
				if (gtable[i][GI_loc[i]].tag == GTAG_loc[i])
				{
					HitBank = i;
					LongestMatchPred = (gtable[HitBank][GI_loc[HitBank]].ctr >= 0);
					break;
				}
		}
		...
		if (HitBank > 0)
		{
			if (AltBank > 0)
			{
				alttaken = (gtable[AltBank][GI_loc[AltBank]].ctr >= 0);
				AltConf = (abs(2 * gtable[AltBank][GI_loc[AltBank]].ctr + 1) > 1);
			}
			else
				alttaken = getbim();
			...
			if ((!Huse_alt_on_na) || (abs(2 * gtable[HitBank][GI_loc[HitBank]].ctr + 1) > 1))
				tage_pred = LongestMatchPred;
			else
				tage_pred = alttaken;

			HighConf =
					(abs(2 * gtable[HitBank][GI_loc[HitBank]].ctr + 1) >=
					 (1 << CWIDTH) - 1);
			LowConf = (abs(2 * gtable[HitBank][GI_loc[HitBank]].ctr + 1) == 1);
			MedConf = (abs(2 * gtable[HitBank][GI_loc[HitBank]].ctr + 1) == 5);
		}
	}

	// compute the prediction
	bool PREDICTOR::GetPrediction(UINT64 PC, bool isprepred)
	{
		// computes the TAGE table addresses and the partial tags

		Tagepred(PC, isprepred);
		...
		// update_since_last_pred = false;
		return pred_taken;
	}

	void PREDICTOR::HistoryUpdate(UINT64 PC, OpType opType, bool taken,
...
#define GINDEX (((long long)PC) ^ bhist ^ (bhist >> (8 - i)) ^ (bhist >> (16 - 2 * i)) ^ (bhist >> (24 - 3 * i)) ^ (bhist >> (32 - 3 * i)) ^ (bhist >> (40 - 4 * i))) & ((1 << (logs - (i >= (NBR - 2)))) - 1)
	int PREDICTOR::Gpredict(UINT64 PC, long long BHIST, int *length,
	...
	void PREDICTOR::Gupdate(UINT64 PC, bool taken, long long BHIST, int *length,
							 int8_t **tab, int NBR, int logs, int8_t *W)
	{
	...
	void PREDICTOR::TrackOtherInst(UINT64 PC, OpType opType, bool taken,
											UINT64 branchTarget)
	{
...
#ifdef LOOPPREDICTOR
	int PREDICTOR::lindex(UINT64 PC)
	{
...
#define CONFLOOP 15
	bool PREDICTOR::getloop(UINT64 PC)
	{
		LHIT = -1;
	...
	void PREDICTOR::loopupdate(UINT64 PC, bool Taken, bool ALLOC)
	{
		if (LHIT >= 0)
		{
